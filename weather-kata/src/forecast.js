...
class Forecast {
  predict(city, datetime, wind) {
    return new Promise((resolve, reject) => {
      ...
      if (datetime < datetime2) {
        ...
        this.getCityId(city, (err, res, body) => {
          const woeid = body[0].woeid;

          // Find the predictions for the city
          this.getCityWeatherData(woeid, (err, res, body) => {
            const results = body.consolidated_weather;
            results.forEach(function (result) {
              // When the date is the expected
              if (
                result.applicable_date == datetime.toISOString().slice(0, 10)
              ) {
  ...
  getCityWeatherData(woeid, onSuccessLoad) {
    request(
